 ...
 *
 * Expressions are compiled to a tiny AST once and cached. The previous implementation reparsed
 * every expression from text on every pixel, which made even small local filters take seconds per
 * frame. Runtime evaluation now only walks the already-validated AST.
 */
class ExpressionEvaluator(
    ...
    fun eval(): Double {
        val compiled = synchronized(CACHE) {
            CACHE[source] ?: Compiler(source).compile().also { CACHE[source] = it }
        }
        val value = compiled.eval(this)
        return if (value.isFinite()) value else 0.0
    }

    private sealed interface Node {
        fun eval(context: ExpressionEvaluator): Double
    }

    private data class ConstantNode(val value: Double) : Node {
        override fun eval(context: ExpressionEvaluator): Double = value
    }

    private data class VariableNode(val name: String) : Node {
        override fun eval(context: ExpressionEvaluator): Double = context.variable(name)
    }

    private data class UnaryNode(val op: Char, val child: Node) : Node {
        override fun eval(context: ExpressionEvaluator): Double {
            val value = child.eval(context)
    ...
    private data class BinaryNode(val op: Char, val left: Node, val right: Node) : Node {
        override fun eval(context: ExpressionEvaluator): Double {
            val a = left.eval(context)
            val b = right.eval(context)
            return when (op) {
                '+' -> a + b
                '-' -> a - b
                '*' -> a * b
    ...
    private data class CallNode(val name: String, val arguments: List<Node>) : Node {
        override fun eval(context: ExpressionEvaluator): Double {
            val values = DoubleArray(arguments.size)
            for (index in arguments.indices) values[index] = arguments[index].eval(context)
            return context.call(name, values)
        }
    }

    private class Compiler(private val source: String) {
        private var pos = 0
        private var steps = 0
        private var depth = 0
        ...
        private fun parseAddSub(): Node {
            ...
            while (true) {
                ...
                skipSpace()
                node = when {
                    take('+') -> BinaryNode('+', node, parseMulDiv())
                    take('-') -> BinaryNode('-', node, parseMulDiv())
                    else -> return node
                }
            }
        ...
        private fun parseMulDiv(): Node {
            ...
            while (true) {
                ...
                skipSpace()
                node = when {
                    take('*') -> BinaryNode('*', node, parsePower())
                    take('/') -> BinaryNode('/', node, parsePower())
                    take('%') -> BinaryNode('%', node, parsePower())
                    else -> return node
                }
        ...
        private fun parsePower(): Node {
            ...
            var node = parseUnary()
            skipSpace()
            if (take('^')) node = BinaryNode('^', node, nested { parsePower() })
            return node
        }
        ...
        private fun parseUnary(): Node {
            ...
            skipSpace()
            return when {
                take('+') -> UnaryNode('+', nested { parseUnary() })
                take('-') -> UnaryNode('-', nested { parseUnary() })
                else -> parsePrimary()
            }
        }
        ...
        private fun parsePrimary(): Node {
            ...
            if (pos < source.length && (source[pos].isDigit() || source[pos] == '.')) {
                return ConstantNode(parseNumber())
            }
    ...
    private fun call(name: String, args: DoubleArray): Double = when (name.lowercase()) {
        "sin" -> sin(arg(args, 0))
        "cos" -> cos(arg(args, 0))
        "tan" -> tan(arg(args, 0))
        ...
        "landmark_distance" -> {
            val p1 = point(arg(args, 0), arg(args, 1))
            val p2 = point(arg(args, 0), arg(args, 2))
    ...
    private fun bounds(groupIndex: Double): Bounds? {
        ...
        return Bounds(
            points.minOf { it.x.toDouble() },
            points.maxOf { it.x.toDouble() },
            points.minOf { it.y.toDouble() },
            points.maxOf { it.y.toDouble() }
        )
    }

    private fun arg(args: DoubleArray, index: Int): Double = if (index in args.indices) args[index] else 0.0
    ...
    companion object {
        ...
        private val CACHE = object : LinkedHashMap<String, Node>(128, 0.75f, true) {
